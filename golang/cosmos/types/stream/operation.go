...
func (so StreamOperation) LoadAndCheckHead(ctx sdk.Context, prior StreamPosition) (*StreamCell, error) {
	head := NewStreamCell(ctx.BlockHeight(), prior.SequenceNumber+1)
	...
	nextSequence := head.FirstSequenceNumber + uint64(len(head.Values))
	if prior.SequenceNumber != nextSequence-1 {
...
func (so StreamOperation) Commit(ctx sdk.Context, priorPos StreamPosition, forceOverwrite bool) error {
	head := NewStreamCell(ctx.BlockHeight(), priorPos.SequenceNumber+1)
	if !forceOverwrite {
	...
	if head.UpdatedBlockHeight != ctx.BlockHeight() {
		...
		head.FirstSequenceNumber += uint64(len(head.Values))
		head.Values = nil
	}
