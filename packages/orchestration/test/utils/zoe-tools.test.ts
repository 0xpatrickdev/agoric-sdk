...
test('withdraw (withdrawToSeat) from LCA with insufficient balance', async t => {
  ...
  await t.throwsAsync(vt.when(E(userSeat).getOfferResult()), {
    message:
      'One or more withdrawals failed. Returned all payments to the source local account. ["[RangeError: -10 is negative]","[RangeError: -10 is negative]"]',
  });
});
...
test('withdrawToSeat, unknown brand', async t => {
  ...
  await t.throwsAsync(vt.when(E(userSeat).getOfferResult()), {
    message:
      'One or more withdrawals failed. Returned all payments to the source local account. ["[Error: key \\"[Alleged: MOO brand]\\" not found in collection \\"brandToAssetRecord\\"]"]',
  });
});
